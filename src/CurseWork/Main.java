...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Сотрудник с максимальной зарплатой: " + workerWitgMaxSalary(workers));
        System.out.println("Средняя зарплата сотрудников за месяц составляет: "+ sumSalary(workers)/workers.length+ " ₽\n");
        nameEmployee(workers);
    }
    ...
    public static Employee workerWitgMaxSalary (Employee[] workers){
        Employee workerMin = workers[0];
        for (Employee worker : workers){
            if(worker.getSalary()>workerMin.getSalary()){
                workerMin = worker;
    ...
    public static void nameEmployee (Employee[] workers){
        for (Employee worker : workers) {
